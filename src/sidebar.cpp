...
void initControlPanel(AppState& state)
{
	...
	state.controlPanelElems.push_back(SidePanelElement(
		...
		[&state](Vector2 topLeft)
		{
			...
			GuiLabel((Rectangle) {topLeft.x + SIDE_PANEL_WIDTH - 100, topLeft.y + 45, 90, 40}, "0");

			float sliderVal = FIELD_WIDTH - state.robotStartX;
			GuiSlider((Rectangle) {topLeft.x + 110, topLeft.y + 50, SIDE_PANEL_WIDTH - 220, 30}, "",
	...
	state.controlPanelElems.push_back(SidePanelElement(
		...
		[&state](Vector2 topLeft)
		{
			...
			GuiSetStyle(DEFAULT, TEXT_ALIGNMENT, TEXT_ALIGN_LEFT);
			GuiLabel((Rectangle) {topLeft.x + SIDE_PANEL_WIDTH - 100, topLeft.y + 45, 90, 40},
					 TextFormat("%.2f", FIELD_HEIGHT));

			GuiSlider((Rectangle) {topLeft.x + 110, topLeft.y + 50, SIDE_PANEL_WIDTH - 220, 30}, "",
					  "", &state.robotStartY, 0.0f, FIELD_HEIGHT);
		},
		110));
	...
	if (state.selectedWaypointIndex != -1)
	{
		...
		state.sidePanel.elems.push_back(SidePanelElement(
			...
			[&state, wpIdx](Vector2 topLeft)
			{
				...
				GuiLabel((Rectangle) {topLeft.x + 10, topLeft.y, SIDE_PANEL_WIDTH - 20, 30},
						 "Heading at point:");

				float oldVal = state.waypoints[wpIdx].heading;
				GuiSlider((Rectangle) {topLeft.x + 10, topLeft.y + 35, SIDE_PANEL_WIDTH - 70, 30},
						  "", TextFormat("%.0f", state.waypoints[wpIdx].heading),
						  &state.waypoints[wpIdx].heading, 0, 360);
		...
		state.sidePanel.elems.push_back(SidePanelElement(
			...
			[](Vector2 tl)
			{
				...
				GuiLabel((Rectangle) {tl.x + 10, tl.y + 5, SIDE_PANEL_WIDTH - 20, 25},
						 "Variable Overrides:");
			},
			30));
		...
		for (int i = 0; i < (int) state.waypoints[wpIdx].variableChanges.size(); i++)
		{
			state.sidePanel.elems.push_back(SidePanelElement(
				...
				[&state, idx = i, wpIdx](Vector2 tl)
				{
					...
					if (GuiButton((Rectangle) {tl.x + SIDE_PANEL_WIDTH - 90, tl.y, 80, 30},
								  "Remove"))
					{
		...
		state.sidePanel.elems.push_back(SidePanelElement(
			...
			[&state](Vector2 topLeft)
			{
				if (GuiButton((Rectangle) {topLeft.x + 10, topLeft.y, SIDE_PANEL_WIDTH - 20, 40},
							  "Set Variable"))
				{
		...
		state.sidePanel.elems.push_back(SidePanelElement(
			...
			[&state, captureIndex = wpIdx](Vector2 topLeft)
			{
				if (GuiButton((Rectangle) {topLeft.x + 10, topLeft.y, SIDE_PANEL_WIDTH - 20, 40},
							  "DELETE WAYPOINT"))
				{
					if (captureIndex >= 0 && captureIndex < (int) state.waypoints.size())
					{
		...
		for (auto& action : state.autoRoutine)
		{
			state.sidePanel.elems.push_back(SidePanelElement(
				...
				[action](Vector2 topLeft)
				{
					...
					GuiLabel((Rectangle) {topLeft.x + 10, topLeft.y, SIDE_PANEL_WIDTH - 20, 50},
							 action.name.c_str());
					...
					std::string codePreview = action.codegen.length() > 28
												  ...
												  : action.codegen;
					GuiLabel(
						(Rectangle) {topLeft.x + 10, topLeft.y + 60, SIDE_PANEL_WIDTH - 20, 40},
						codePreview.c_str());
