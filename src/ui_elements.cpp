...
void TopBar::draw()
{
	...
	for (size_t i = 0; i < elems.size(); i++)
	{
		...
		elems[i].execute((Vector2) {static_cast<float>(currentX), (float) (height - 40) / 2});
		currentX += elems[i].width;
	}
}

SidePanelElement::SidePanelElement(std::string label, std::function<void(Vector2)> execute,
								   int height)
...
void SidePanel::draw(int yOffset)
{
	...
	for (size_t i = 0; i < elems.size(); i++)
	{
		...
		elems[i].execute(
			(Vector2) {static_cast<float>(screenWidth - width), static_cast<float>(totalHeight)});
		if (i >= elems.size())
