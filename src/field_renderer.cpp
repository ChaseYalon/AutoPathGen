...
void drawWaypoints(AppState& state) {
	for (size_t i = 0; i < state.waypoints.size(); ++i) {
		...
		bool isHovered = CheckCollisionPointCircle(
			GetMousePosition(), state.waypoints[i].pos, 45.0f
		);
		...
		Vector2 endPos =
			{state.waypoints[i].pos.x + lineLen * cosf(angleRad),
	...
	if (state.placingWaypoint && !state.showAddActionForm && !state.showAddVariableForm &&
		...
		if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
			if (state.driveActions.empty()) {
				...
				// exit(1); // Don't crash
				state.addAlert(
					"Create a drive action before placing a waypoint!", AlertType::Error
				);
			...
			if (mousePos.y > state.topBar.height && mousePos.x < state.fieldImageWidth) {
				...
				if (idx == 0) {
					...
					state.waypoints[0].handleIn =
						{start.x + delta.x * 0.75f, start.y + delta.y * 0.75f};
					state.robotState.handleOut =
						{start.x + delta.x * 0.25f, start.y + delta.y * 0.25f};
				} else {
					Vector2 start = state.waypoints[idx - 1].pos;
					Vector2 end = state.waypoints[idx].pos;
					Vector2 delta = {end.x - start.x, end.y - start.y};

					state.waypoints[idx - 1].handleOut =
						{start.x + delta.x * 0.25f, start.y + delta.y * 0.25f};

					state.waypoints[idx].handleIn =
						{start.x + delta.x * 0.75f, start.y + delta.y * 0.75f};
				}
