...
void handleWaypointPlacement(AppState &state) {
  // Handle placement
  if (state.placingWaypoint && !state.showAddActionForm &&
      !state.showAddVariableForm && !state.showAddActionToWaypointForm &&
      !state.askForStartingPosition) {
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
      ...
      Vector2 mousePos = GetMousePosition();
      if (mousePos.y > state.topBar.height &&
          mousePos.x < state.fieldImageWidth) {
