...
void drawStartingPositionPopup(AppState &state) {
  ...
  const float popupY =
      ((state.fieldImageHeight - popupHeight) / 2.0f) + state.topBar.height;
  ...
  if (GuiTextBox((Rectangle){popupX + 100, popupY + 80, 200, 40},
                 state.robotStartXText, 32, state.startXEditMode))
  ...
  if (GuiTextBox((Rectangle){popupX + 100, popupY + 150, 200, 40},
                 state.robotStartYText, 32, state.startYEditMode))
...
void drawAddActionPopup(AppState &state) {
  ...
  int totalVars =
      (int)state.providedVariables.size() + (int)state.customVariables.size();
  ...
  if (GuiTextBox(
          ...
          state.actionName, 128, state.actionNameEditMode)) {
  ...
  if (GuiTextBox(
          ...
          state.actionCode, 1024, state.actionCodeEditMode)) {
  ...
  for (size_t i = 0; i < state.providedVariables.size(); i++) {
    ...
    GuiLabel((Rectangle){xPos, varY, 380, 30},
             state.providedVariables[i].name.c_str());
    if (varCount % 2 == 1)
      varY += 30;
    varCount++;
  }
  ...
  if (GuiButton(
          ...
          "Save Action")) {
    state.actions.push_back(Action(state.actionName, state.actionCode,
                                   detectVariables(state.actionCode)));
  ...
  if (GuiButton(
          ...
          "Save Drive Action")) {
    state.driveActions.push_back(Action(state.actionName, state.actionCode,
                                        detectVariables(state.actionCode)));
    state.showAddActionForm = false;
    state.actionName[0] = '\0';
    state.actionCode[0] = '\0';
  ...
  for (size_t i = 0; i < state.actions.size(); i++) {
    ...
    if (params != wasChecked) {
      if (params) {
        ...
        for (size_t k = 0;
             k <
             state.waypoints[state.selectedWaypointIndex].boundActions.size();
             k++) {
          if (state.waypoints[state.selectedWaypointIndex]
                  .boundActions[k]
                  .name == state.actions[i].name) {
...
void drawAddVariablePopup(AppState &state) {
  ...
  if (GuiTextBox(
          (Rectangle){popupX + 20, popupY + 90, popupWidth - 40, 40},
          state.variableName, 128, state.variableNameEditMode)) {
  ...
  if (state.customVariables.empty()) {
    ...
    if (GuiButton(
      ...
      if (selectedVarIndex >= 0 &&
        ...
        if (!found) {
          state.waypoints[state.selectedWaypointIndex]
              .variableChanges.push_back(VariableChange(varName, val));
        }
    ...
    if (GuiDropdownBox((Rectangle){popupX + 20, popupY + 90, 300, 40},
                       comboText.c_str(), &selectedVarIndex, dropdownEdit)) {
      dropdownEdit = !dropdownEdit;
    }
    if (oldIdx != selectedVarIndex && selectedVarIndex >= 0 &&
        selectedVarIndex < (int)state.customVariables.size()) {
      strncpy(valBuf, state.customVariables[selectedVarIndex].value.c_str(),
              sizeof(valBuf));
    }
  }
