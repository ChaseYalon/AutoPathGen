...
Action::Action(std::string name, std::string codegen,
               std::vector<ProvidedVariable> usedVariables)
    : name(std::move(name)), codegen(codegen), originalTemplate(codegen),
...
void Robot::draw() {
  ...
  Vector2 endPos = {x + lineLen * cosf(angleRad),
                    y + lineLen * sinf(angleRad)};
