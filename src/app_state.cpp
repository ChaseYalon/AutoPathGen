...
AppState::AppState()
    : fieldImageWidth(0), fieldImageHeight(0), robotStartX(0), robotStartY(0),
      startXEditMode(false), startYEditMode(false),
      askForStartingPosition(true), showAddActionForm(false),
      showAddActionToWaypointForm(false), showAddVarChangeForm(false),
      selectedActionTemplateIndex(-1), actionDropdownEditMode(false),
      placingWaypoint(false), viewingActions(false),
      selectedWaypointIndex(-1), actionNameEditMode(false),
      actionCodeEditMode(false), showAddVariableForm(false),
      variableNameEditMode(false), variableValueEditMode(false) {
