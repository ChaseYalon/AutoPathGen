...
WaypointContext calculateContext(const AppState& state, size_t i) {
	...
	if (i == 0) {
		...
		prevPixelPos =
			{state.waypoints[i - 1].pos.x,
			 state.waypoints[i - 1].pos.y - state.topBar.height};
	}
	Vector2 currentPixelPos =
		{state.waypoints[i].pos.x, state.waypoints[i].pos.y - state.topBar.height};

	Vector2 startField = pixelsToWpilibCoords(
		prevPixelPos,
		FIELD_WIDTH,
		FIELD_HEIGHT,
		state.fieldImageWidth,
	...
	Vector2 endField = pixelsToWpilibCoords(
		currentPixelPos,
		FIELD_WIDTH,
		FIELD_HEIGHT,
		state.fieldImageWidth,
	...
	if (i + 1 < state.waypoints.size()) {
		Vector2 nextPixel =
			{state.waypoints[i + 1].pos.x,
			 state.waypoints[i + 1].pos.y - state.topBar.height};
		Vector2 temp = pixelsToWpilibCoords(
			nextPixel,
			FIELD_WIDTH,
			FIELD_HEIGHT,
			state.fieldImageWidth,
...
std::string applyContext(
	std::string code,
	const WaypointContext& ctx,
...
void rebuildAutoRoutine(AppState& state) {
	...
	// Robot Start State
	Vector2 pixelPosPrev = wpilibCoordsToPixels(
		state.robotStartX,
		state.robotStartY,
		FIELD_WIDTH,
		FIELD_HEIGHT,
	...
	for (size_t i = 0; i < state.waypoints.size(); ++i) {
		...
		for (int s = 1; s <= N; ++s) {
			...
			if (s == N) {
				...
				Vector2 pBeforeField = pixelsToWpilibCoords(
					pixelPosPrev,
					FIELD_WIDTH,
					FIELD_HEIGHT,
					state.fieldImageWidth,
				...
				Vector2 pAfterField = pixelsToWpilibCoords(
					posPixels,
					FIELD_WIDTH,
					FIELD_HEIGHT,
					state.fieldImageWidth,
				...
				// Robot Heading: 0 is North (Up), 90 is East (Right) (if matching
				// compass) Or does it match standard math? Let's check Robot::draw: float
				// angleRad = (r - 90) * ... If r=90, angleRad=0 (Right). So Heading 90 is
				// East. If math angle is 0 (East), Heading should be 90. If math angle is
				// 90 (North), Heading should be 0. Heading = 90 - MathAngle.

				heading = 90.0f - angleDeg;

				// Normalize to 0-360 if desired, but not strictly needed for codegen
				// contexts unless specified
			}
			...
			Vector2 nextField = pixelsToWpilibCoords(
				nextPosPixels,
				FIELD_WIDTH,
				FIELD_HEIGHT,
				state.fieldImageWidth,
			...
			if (!state.driveActions.empty()) {
				...
				driveAct.codegen = applyContext(
					driveAct.originalTemplate, ctx, currentVariables
				);
		...
		Vector2 currentField = pixelsToWpilibCoords(
			pixelPosPrev,
			FIELD_WIDTH,
			FIELD_HEIGHT,
			state.fieldImageWidth,
		...
		finalCtx.distBlue = sqrtf(
			powf(currentField.x - 4.6f, 2) + powf(currentField.y - 4.0f, 2)
		);
		finalCtx.distRed = sqrtf(
			powf(currentField.x - 12.0f, 2) + powf(currentField.y - 4.0f, 2)
		);
		...
		for (const auto& actionTemplate : state.waypoints[i].boundActions) {
			...
			instance.codegen = applyContext(
				instance.originalTemplate, finalCtx, currentVariables
			);
			state.autoRoutine.push_back(instance);
			TraceLog(
				LOG_INFO, "Added bound action %s at WP %d", instance.name.c_str(), (int) i
			);
		}
	}
}
